...
public abstract class AbstractGameplayScreenController implements StageAware, Initializable {
    ...
    @Override public void initialize(URL location, ResourceBundle resources) { versionLabel.setText("dev"); syncScaleLabel(); if (actionLayer != null) actionLayer.setPickOnBounds(false); viewCoordinator = new GameplayViewCoordinator(toUiBundle()); viewCoordinator.setOnHandCardPick(this::handleHandCardPick); installJustSayNoHandler(); installPaymentPicker(); AppContext.get().networkLobbyState().addListener(msg -> { if (msg != null && msg.getType() == com.example.monopoly_deal_game.network.NetworkMessage.Type.SESSION_SNAPSHOT && msg.getSession() != null) { javafx.application.Platform.runLater(() -> { AppContext.get().gameEngine().resumeSession(msg.getSession()); selectedHandCard = null; discardSelections.clear(); refreshGameplayUi(); }); } }); if (undoButton != null) { undoButton.setDisable(true); undoButton.setTooltip(new Tooltip("暂未开放：原计划用于撤回上一次打出的牌并恢复棋盘状态。\n当前对局无快照机制，为避免误触已禁用该按钮。")); } if (gameRoot != null) gameRoot.setStyle("-fx-focus-color: transparent; -fx-faint-focus-color: transparent;"); refreshGameplayUi(); }
    private void installJustSayNoHandler() { JustSayNoMediator.installUi(this::promptJustSayNoDialog); }
    private void installPaymentPicker() { PaymentPickerMediator.installUi((payer, amountDueM, receiver, session, reasonText) -> PaymentPickDialogs.choosePaymentCards(stage, payer, amountDueM, receiver, session, reasonText)); }
    private boolean promptJustSayNoDialog(Player respondent, Player activator, String situation) { var useNo = new ButtonType("使用 Just Say No（反对）", ButtonBar.ButtonData.APPLY); Alert alert = new Alert(Alert.AlertType.CONFIRMATION, situation + "\n\n你的「反对（Just Say No）」在手牌或银行中。\n• 若使用：当场弃掉该张反对，并抵消本条仅对你的效果。\n• 若不使用：结算照常继续。", useNo, new ButtonType("不使用反对（继续结算）", ButtonBar.ButtonData.CANCEL_CLOSE)); alert.setTitle("Just Say No"); if (stage != null) alert.initOwner(stage); String act = activator != null ? activator.getName() : "对方"; alert.setHeaderText(respondent.getName() + " — 是否要反对「" + act + "」的举动？"); return alert.showAndWait().filter(r -> r == useNo).isPresent(); }
    ...
    private void setFeedback(String msg) { if (feedbackLabel != null) feedbackLabel.setText(msg == null ? "" : msg); }
    ...
    private void applyHud(GameSession session) {
        ...
        switch (state.getPhase()) {
            case DRAW_PHASE -> {
                ...
                if (myTurn && feedbackLabel != null && (feedbackLabel.getText() == null || feedbackLabel.getText().isBlank())) {
                    feedbackLabel.setStyle("-fx-font-size: 10px; -fx-text-fill: #1565c0; -fx-padding: 2 0 4 0;");
                    feedbackLabel.setText("提示：每回合须先摸牌");
                }
            }
            case PLAY_PHASE -> {
            ...
            case DISCARD_PHASE -> {
                ...
                if (myTurn && feedbackLabel != null && (feedbackLabel.getText() == null || feedbackLabel.getText().isBlank())) {
                    feedbackLabel.setStyle("-fx-font-size: 10px; -fx-text-fill: #e65100; -fx-padding: 2 0 4 0;");
                    feedbackLabel.setText("弃牌阶段：点击手牌多选，弃至手牌 ≤7 张；亮边与略放大的即为已选。");
                }
            }
        }
    }
    ...
    private void publishSessionChange(GameSession session) { if (session == null) return; String roomId = AppContext.get().networkLobbyState().getRoomId(); if (roomId == null || roomId.isBlank()) return; if (AppContext.get().networkLobbyState().isHost()) { GameServer.Room room = AppContext.get().gameServer().getRoom(roomId); if (room != null) room.broadcastSessionSnapshot(); return; } var client = AppContext.get().networkLobbyState().getClient(); if (client == null) return; try { client.send(com.example.monopoly_deal_game.network.NetworkMessage.builder(com.example.monopoly_deal_game.network.NetworkMessage.Type.PLAYER_ACTION).roomId(roomId).playerName(AppContext.get().networkLobbyState().getLocalPlayerName()).session(session).build()); } catch (Exception ex) { setFeedback("同步操作失败：" + ex.getMessage()); } }
    ...
    private void updatePlayChrome(GameSession session) { removePlayChromeLayoutListener(); actionLayer.getChildren().clear(); if (session == null || selectedHandCard == null) { actionLayer.setMouseTransparent(true); actionLayer.setPickOnBounds(false); return; } GameState st = session.getGameState(); if (st.isGameOver() || st.getPhase() != GameState.Phase.PLAY_PHASE) { actionLayer.setMouseTransparent(true); actionLayer.setPickOnBounds(false); return; } if (!requiresExplicitPlayMode(selectedHandCard)) { actionLayer.setMouseTransparent(true); actionLayer.setPickOnBounds(false); return; } actionLayer.setMouseTransparent(false); actionLayer.setPickOnBounds(false); GameLogic logic = AppContext.get().gameEngine().getGameLogic(); var me = session.getCurrentPlayer(); if (me == null) { actionLayer.setMouseTransparent(true); actionLayer.setPickOnBounds(false); return; } VBox dock = new VBox(10); dock.setAlignment(Pos.CENTER); dock.setPadding(new Insets(10, 16, 10, 16)); dock.setPickOnBounds(true); dock.setStyle("-fx-background-color: rgba(25,118,210,0.94); -fx-background-radius: 10; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.25), 12, 0, 0, 2);"); Label bar = new Label("出牌选项（可点击其他手牌取消或重选）"); bar.setStyle("-fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: 12px; -fx-padding: 0 0 2 0;"); HBox row = new HBox(12); row.setAlignment(Pos.CENTER); Card sel = selectedHandCard; if (sel instanceof PropertyCard pc) { Button toTable = new Button("摆上桌面（作房产）"); toTable.setStyle("-fx-font-size:12px;"); toTable.setOnAction(e -> tryPlayWithOptions(session, CardPlayOptions.auto())); Button toBank = new Button("存入银行（现金 " + Math.max(0, sel.getValue()) + "M）"); toBank.setStyle("-fx-font-size:12px;"); toBank.setOnAction(e -> tryPlayWithOptions(session, CardPlayOptions.bankOnly())); row.getChildren().addAll(toTable, toBank); if (pc.canFlipWildDualColor()) { Button flipDual = new Button("切换双色（紫/橙等）"); flipDual.setStyle("-fx-font-size:12px;"); flipDual.setOnAction(e -> { pc.switchColor(); refreshGameplayUi(); }); row.getChildren().add(flipDual); } dock.getChildren().addAll(bar, row); } else if (sel instanceof ActionCard ac) { Button use = new Button("使用卡牌效果"); use.setStyle("-fx-font-size:12px;"); boolean can = logic.getEffectExecutor().canUseActionEffectForUi(ac, session); use.setDisable(!can); if (!can) use.setTooltip(new Tooltip("当前没有可生效的对手或前提不满足；可把牌作现金存入银行。")); use.setOnAction(e -> tryPlayWithOptions(session, CardPlayOptions.auto())); Button toBank = new Button("存入银行（现金 " + Math.max(0, ac.getValue()) + "M）"); toBank.setStyle("-fx-font-size:12px;"); toBank.setOnAction(e -> tryPlayWithOptions(session, CardPlayOptions.bankOnly())); row.getChildren().addAll(use, toBank); dock.getChildren().addAll(bar, row); } else if (sel instanceof RentCard rc) { Button toBank = new Button("存入银行（现金 " + Math.max(0, rc.getValue()) + "M）"); toBank.setStyle("-fx-font-size:12px;"); toBank.setOnAction(e -> tryPlayWithOptions(session, CardPlayOptions.bankOnly())); row.getChildren().add(toBank); java.util.List<CardColor> colors = RentRules.eligibleChargeColors(rc, me); FlowPane fp = new FlowPane(); fp.setAlignment(Pos.CENTER); fp.setHgap(8); fp.setVgap(8); if (colors.isEmpty()) { Label empty = new Label("当前没有可收租的颜色"); empty.setStyle("-fx-text-fill: white; -fx-font-size: 12px;"); fp.getChildren().add(empty); } else { boolean dbl = st.isDoubleNextRent(); for (CardColor col : colors) { int base = RentCalculator.rentOnColor(me, col); int display = dbl ? base * 2 : base; Button cb = new Button("收租 " + rentColorShortLabel(col) + " → " + display + "M"); cb.setStyle("-fx-font-size:11px;"); CardColor cap = col; boolean rentBtnOk = RentRules.canUseRentEffectForUi(rc, me, cap, session); cb.setDisable(!rentBtnOk); cb.setOnAction(e -> tryPlayWithOptions(session, CardPlayOptions.rentWithColor(cap))); fp.getChildren().add(cb); } } dock.getChildren().addAll(bar, row, fp); } actionLayer.getChildren().add(dock); playChromeLayoutListener = obs -> layoutPlayChromeDock(dock); actionLayer.widthProperty().addListener(playChromeLayoutListener); actionLayer.heightProperty().addListener(playChromeLayoutListener); layoutPlayChromeDock(dock); }
