...
public class PlayChromeBuilder {
    ...
    public VBox build(GameSession session, Card selectedCard,
        ...
        if (selectedCard instanceof RentCard rc) {
            buildRentChrome(dock, bar, row, rc, me, session, st, onPlay);
        } else if (selectedCard instanceof PropertyCard pc) {
            buildPropertyChrome(dock, bar, row, pc, selectedCard, onPlay, refreshUi);
        } else if (selectedCard instanceof ActionCard ac) {
            buildActionChrome(dock, bar, row, ac, session, logic, onPlay);
        } else {
            return null;
        }
    ...
    public static boolean requiresExplicitPlayMode(Card c) {
        return c instanceof ActionCard || c instanceof RentCard || c instanceof PropertyCard;
    }
