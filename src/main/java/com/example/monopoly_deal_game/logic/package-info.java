 ...
 * 对应架构图核心：{@link com.example.monopoly_deal_game.game.engine.GameEngine} 依赖 {@link com.example.monopoly_deal_game.logic.GameLogic}；
 * {@link com.example.monopoly_deal_game.network.GameServer} 在主机侧也应调用同一套逻辑以保证权威状态。
 * </p>
 *
 * <h3>类与需求映射（方便分工）</h3>
 * <ul>
 *   <li>{@link com.example.monopoly_deal_game.game.rules.GameConfig} — 常量：人数上下限、起手5张、回合摸2/5张、手牌上限7、每回合出牌≤3等。</li>
 ...
 *   <li>{@link com.example.monopoly_deal_game.game.engine.GameEngine} — {@code launchGame}、{@code startLocalGame}、{@code startLanGame} 入口，组装上面对象。</li>
 * </ul>
 *
 * <h3>与其它包的关系</h3>
 * <ul>
 *   <li><b>→ model</b>：只修改 {@link com.example.monopoly_deal_game.game.model.GameSession} 内数据，不直接操作 JavaFX。</li>
 *   <li><b>← controller</b>：控制器只调 {@link com.example.monopoly_deal_game.game.engine.GameEngine} 的用例方法。</li>
 ...
 * <h3>TODO（总体）</h3>
 * <ul>
 *   <li>详见本包目录下 {@code LOGIC_TASKS.md}（分工清单、前置条件、与各需求章节映射）。</li>
