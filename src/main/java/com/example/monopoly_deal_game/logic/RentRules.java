...
public final class RentRules {
    ...
    public static List<CardColor> eligibleChargeColors(RentCard rc, Player landlord) {
        ...
        for (CardColor c : rc.getApplicableColors()) if (c != null && c != CardColor.NONE && c != CardColor.WILD && RentCalculator.rentOnColor(landlord, c) > 0) out.add(c);
        return out;
    }
    public static boolean canUseRentEffect(RentCard rc, Player landlord, CardColor declared) {
        List<CardColor> ok = eligibleChargeColors(rc, landlord); if (ok.isEmpty()) return false; return declared == null || ok.contains(declared);
    }
    public static boolean canUseRentEffect(RentCard rc, Player landlord, CardColor declared, Player payerChosen, GameSession session) {
        ...
        if (payerChosen != null) return renters.contains(payerChosen); return renters.size() == 1;
    }
    ...
    public static Player resolvedRentPayer(Player landlord, GameSession session, CardPlayOptions opt) {
        List<Player> renters = rentersExcludingLandlord(landlord, session); if (renters.isEmpty()) return null; Player pick = opt != null ? opt.actionTargetPlayer() : null; if (pick != null && renters.contains(pick)) return pick; if (renters.size() == 1) return renters.get(0); if (landlord != null && landlord.isAI()) return renters.get(0); return null;
    }
}
